class Users::SessionsController < Devise::SessionsController
  ...
  def create
    ...
    if current_user.role == User.role_users[:customer]
      ...
      cart = Cart.create(user_id: current_user.id) unless cart.present?
      if cookies[:cart].present?
        list_items = JSON.parse(cookies[:cart])
        list_items.each do |record|
          cart_item =
            CartItem.find_by(item_id: record[0].to_i, cart_id: cart.id)
          if cart_item.present?
            cart_item.update_columns(
              quantity: (cart_item.quantity + record[1].to_i)
            )
          else
